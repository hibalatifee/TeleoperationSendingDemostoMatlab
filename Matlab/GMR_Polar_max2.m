function [y, Sigma_y] = GMR_Polar_max2(Priors, Mu, Sigma, x, in, out,P_Dim,Beta)
%
...
for i=1:nbStates
    ...
    Pxi(:,i) = (Priors(i).*gaussPDF(Data2(:,:,i), Mu(:,i), Sigma(:,:,i))).^Beta;
    %Nearest_Neighbour
%     Pxi(:,i) = sum((Data2(:,:,i) - Mu(in,i)).^2);
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for j=1:nbStates
  y_tmp(:,:,j) = repmat(Mu(out,j),1,nbData) + Sigma(out,in,j)*inv(Sigma(in,in,j)) * (Data2(in,:,j)-repmat(Mu(in,j),1,nbData));
end
...
%     y = atan(s_mean/c_mean);
    y = zeros(1,length(c_mean));
   for i2=1:length(c_mean) 
        if(c_mean(i2)<0)
            y(i2) = atan(s_mean(i2)/c_mean(i2))+pi;
        elseif(s_mean(i2)>0)
            y(i2) = atan(s_mean(i2)/c_mean(i2));
        else
            y(i2) = atan(s_mean(i2)/c_mean(i2))+2*pi;
        end
   end
end
